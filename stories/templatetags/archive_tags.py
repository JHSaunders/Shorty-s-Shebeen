...
def get_query_string(p, new_params, remove, context,as_hidden=False):
    """
    ...
    if not as_hidden:           
        ...
        return mark_safe("\n".join([u'<input type="hidden" name="%s" value="%s"/>' % (k, cgi.escape(unicode(v))) for k, v in p.items()]))
...
@register.tag
def query_string_as_hiddens(parser, token):
    """
    Allows you too manipulate the query string of a page by adding and removing keywords
    and create all the values as hidden fields.
    If a given value is a context variable it will resolve it.
    Based on similiar snippet by user "dnordberg".
    ...
    return QueryStringAsNode(add,remove)

class QueryStringAsNode(template.Node):
    def __init__(self, add,remove):
        self.add = add
        self.remove = remove
    ...
    def render(self, context):
        ...
        for k, v in context["request"].GET.items():
            p[k]=v
        return get_query_string(p,self.add,self.remove,context,True)
